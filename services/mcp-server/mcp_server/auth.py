...
def authenticate(token: Optional[str]) -> Dict[str, Any]:
    ...
    if not tenant_info:
        logger.warning(f"Authentication failed: Invalid token {masked_token}")
        raise MCPError(
            code=MCPErrorCode.UNAUTHORIZED.value,
    ...
    # Log successful authentication (but don't log the actual token)
    logger.info(f"Authentication successful for tenant: {tenant_info['tenant_id']} (token: {masked_token})")
