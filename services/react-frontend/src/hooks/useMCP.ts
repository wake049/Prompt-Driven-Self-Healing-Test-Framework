...
export function useMCPConnection() {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      setClient(mcpClient);
      setIsConnected(true);
      console.log("✅ MCP connection established in React");
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to connect to MCP server';
      setError(errorMessage);
      setIsConnected(false);
      setClient(null);
      console.error("❌ MCP connection failed in React:", errorMessage);
    } finally {
  ...
  const scheduleReconnect = useCallback(() => {
    ...
    reconnectTimeoutRef.current = setTimeout(() => {
      if (!isConnected && !isConnecting) {
        console.log("🔄 Attempting MCP reconnection from React");
        connect();
      }
    }, 5000);
...
export function useMCPQuery<T = any>(
  ...
  const execute = useCallback(async () => {
    ...
    try {
      ...
      setError(errorMessage);
      console.error("MCP query error:", errorMessage);
    } finally {
      setLoading(false);
    }
