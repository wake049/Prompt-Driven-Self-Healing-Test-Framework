...
const SyncDebugger: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const promptsResponse = await fetch('http://localhost:8000/api/v1/prompts?limit=100', {
      ...
      if (promptsResponse.ok) {
        ...
        for (const prompt of promptsData.data || []) {
          try {
            const testPlanResponse = await fetch(`http://localhost:8000/generated-test-plans/by-prompt/${prompt.id}`, {
              headers: { 'Authorization': `Bearer ${token}` },
            });
      ...
      console.error('Error loading sync debug data:', error);
    } finally {
      setLoading(false);
    }
  };
