...
const SelectedSelector = styled.div<{ type: 'css' | 'xpath' }>`
  background: ${props => props.type === 'css' ? '#dbeafe' : '#fef3c7'};
  color: ${props => props.type === 'css' ? '#1e40af' : '#92400e'};
  padding: 8px 12px;
  border-radius: 6px;
  font-family: 'Monaco', 'Cascadia Code', 'Roboto Mono', monospace;
  font-size: 12px;
  border-left: 3px solid ${props => props.type === 'css' ? '#3b82f6' : '#f59e0b'};
  margin-bottom: 8px;
`;
...
const PolicyStatus = styled.div<{ preference: string }>`
  background: ${props => props.preference === 'css' ? 
    'linear-gradient(135deg, #dbeafe, #bfdbfe)' : 
    'linear-gradient(135deg, #fef3c7, #fde68a)'};
  color: ${props => props.preference === 'css' ? '#1e40af' : '#92400e'};
  padding: 8px 12px;
  border-radius: 8px;
  font-size: 12px;
  font-weight: 600;
  text-align: center;
  margin-bottom: 16px;
  border: 1px solid ${props => props.preference === 'css' ? '#93c5fd' : '#fbbf24'};
`;
