...
export const DashboardAnalytics: React.FC<DashboardAnalyticsProps> = ({ className }) => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      console.log('🐛 Analytics Debug - Raw API Responses:');
      console.log('📊 Trends Data:', JSON.stringify(trendsData, null, 2));
      console.log('💥 Failure Data:', JSON.stringify(failureData, null, 2));
      ...
      console.log('🐛 Analytics Debug - Set State Values:');
      console.log('📈 Trends:', trendsData?.trends || []);
      console.log('🔥 Failure Patterns:', failureData?.failure_patterns || []);
      console.log('📊 Action Failure Rates:', failureData?.action_failure_rates || []);
      console.log('⚡ Performance Metrics:', performanceData || null);
      
      console.log('🐛 Analytics Debug - Data Structure Check:');
      console.log('trends property exists:', 'trends' in (trendsData || {}));
      console.log('failure_patterns property exists:', 'failure_patterns' in (failureData || {}));
  ...
  const calculateMetrics = () => {
    ...
    const avgDuration = trends.reduce((sum, trend) => sum + (trend.avg_duration_ms / 1000), 0) / trends.length; // Convert ms to seconds
  ...
  return (
    <DashboardContainer className={className}>
      ...
      <MetricsGrid>
        ...
        {performanceMetrics && performanceMetrics.p95_execution_time !== undefined && (
          <MetricCard>
            <MetricTitle>P95 Duration</MetricTitle>
            <MetricValue>{formatDuration(performanceMetrics.p95_execution_time / 1000)}</MetricValue>
