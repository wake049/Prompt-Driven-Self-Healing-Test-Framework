...
class SelectorConverter:
    ...
    def css_to_xpath(css_selector: str) -> str:
        ...
        try:
            ...
            # Note: Removed problematic attribute selector regex that was doubling @ characters
            # CSS attribute selectors like [name='value'] are less common and can be handled separately if needed
        ...
        except Exception as e:return f"//*[@data-testid='{css_selector}']"  # Fallback
    ...
    def xpath_to_css(xpath_selector: str) -> str:
        ...
        except Exception as e:return f"[data-testid='{xpath_selector}']"  # Fallback
    ...
    def convert_ai_step_to_dual_selector(step: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        # Check if step already has dual_selectors (from Chrome extension)
        if 'dual_selectors' in step and step['dual_selectors']:
            existing_dual = step['dual_selectors']
            if existing_dual.get('css_selector') and existing_dual.get('xpath_selector'):
                # Both selectors already provided - use them as-is}', XPath='{existing_dual.get('xpath_selector')}'")
                return step
        
        # Extract selector from various possible fields only if dual_selectors not already present
        ...
        if not original_selector:return step
        
        # Create dual selectors only if not already present
...
def convert_steps_to_dual_selector_format(steps: list) -> list:
    ...
    for i, step in enumerate(steps):
        ...
        except Exception as e:converted_steps.append(step)  # Keep original if conversion fails
