...
class SafetyPolicyService:
    ...
    async def check_user_admin_status(self, current_user: CurrentUser) -> bool:
        ...
        try:
            ...
            logger.info(f"🔐 Admin check for user {current_user.user.email}: {is_admin}")
            return is_admin
    ...
    async def validate_destructive_action(
        ...
        if action_lower in self.destructive_actions:
            ...
            if not is_admin:
                logger.warning(
                    f"🚫 SECURITY BLOCK: Non-admin user {current_user.user.email} "
                    f"attempted destructive action: {action_type}"
                    f"{f' in context: {context}' if context else ''}"
                )
                raise HTTPException(
            ...
            logger.info(
                f"✅ SECURITY PASS: Admin user {current_user.user.email} "
                f"authorized for destructive action: {action_type}"
                f"{f' in context: {context}' if context else ''}"
            )
