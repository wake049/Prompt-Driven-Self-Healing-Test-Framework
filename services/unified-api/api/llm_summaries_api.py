...
async def generate_execution_summary(
    ...
    """
    try:
        logger.info(f"🤖 Generating LLM summary for execution {execution_id}")
        ...
        logger.error(f"❌ Failed to generate summary for execution {execution_id}: {str(e)}")
    ...
    try:
        ...
        for execution_id in execution_ids[:10]:  # Limit to 10 for performance
            try:
                ...
                logger.warning(f"⚠️ Failed to generate summary for {execution_id}: {str(e)}")
                continue
...
async def get_execution_details(db: DatabaseManager, execution_id: str, current_user: CurrentUser) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Could not load binding info: {str(e)}")
...
"""
        ...
        try:
            ...
            logger.warning(f"LLM generation failed, using fallback: {str(llm_error)}")
...
async def store_summary_cache(execution_id: str, summary_text: str, insights: List[str], overview: Dict[str, Any]):
    ...
    try:
        ...
        logger.info(f"💾 Cached summary for execution {execution_id}")
